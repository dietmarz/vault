include::{root}/.inc/includeDaily.adoc[]

= 2023-02-16

== Aufgaben 
* WVL hat Lynn geantwortet? Matthias?
* Ortho bezahlen und prüfen
* Locale Admin Rechte für FirmewareUpate für docker 3 updaten


=== 23456 Admin Deployments
* https://jira.int.kn/browse/KNOMP-30401[Subtask]
* search: https://gitlab.int.kn/kn-esp/esp-search/-/pipelines/174619




=== 29936 Operational NPE in knloginedi
* [X] Fix der einzigen Möglichkeit


== Idle
* [ ] Orderline deletion queues learning aufschreiben
* [ ] Eigene Q mit mehreren Topics
* [ ] States durchspielen KNESP
* [ ] Reporting doc lesen
* [ ] tms-export als svg
* [ ] Tobi fragen:
  ** Urlaub beantragen 27.11. - 15.12.23
  ** Klärung Überstunden, wie abbauen



















// '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

== Private Aufgaben

* Mail an Plath verfassen

.[.red]#Weiter#
  ****
* [ ] 10 Linas Sachen vom Dachboden holen
* [ ] 10 Mülltonne reduzieren

  ****

.[.green]#Backlog#
  ****

* 120 Mein Miro https://miro.com/app/board/o9J_lruDyUo=/ umziehen nach board.drawio.svg
  ** Daraus OKRs generieren


* 60 Schreibtisch frei
* 60 Depot kündigen
* 30 TimeBox Keller aufräumen
* OtF Harte Matratze
* Gelbe Säcke organisieren

* neuer Urlaub einstellen 27.11.2023 - 15.12.2023 (Whatsapp vom 18.12)
* Andree de B Aufstellung verstehen

* Zu Projekten: nextcloud with kibana... see techteddy
  ****

== Notizen


|===
| Mail Privat |Cal Privat |Müll  |Sport  |Alarm |Buchen
| Mail Custom |Cal Custom |Brief |Chat   |Daily |
|===

