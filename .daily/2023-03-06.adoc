...
=== 30085 testen The container 'CSLU1540650' is missing a sequence number.
* Core Change:
.SeaShipmentKnieMessageImportMapper
----
called by SeaShipment map(SeaShipmentKnieMessage knieMessage
called by @AfterMapping  default void addOrUpdateContainers
private Container mapToShipmentContainer(ContainerDetails containerDetails, OriginSystem originSystem) { ...
