...
* [ ] Eigru: Aufgaben aus fehlenden Zahlen ermitteln WVL Mail an Raveling Wertfeststellung
* [ ] Board: Fehlende Tasks anbringen, priorisieren
* Akku für ebook reader
...
* Ich bekomme dort ein 401.
* [ ] Doku schreiben!
* [ ] Dev-launcher zurück bauen https://git.int.kn/projects/KNLOGIN/repos/esp-dev-launcher/pull-requests/156/overview

* Versuch int direkt zu benutzen.
  ** Token wurde nicht mitgeschickt
  ** Cookie selber mit typescript-cookie setzen
  ** document.cookie auf der konsole leer
  ** document.cookie hartverdrahtet selber setzen
  ** webDev proxy server nutzen, Kopie vom Frontend





== Local docker / Reporting / Search
* [ ] Test!

== Integration
* [ ] Trotz login-endpoint call mit 200 liefert allocated-orderlines 401 Unauthorized
* [ ] Cookie ss-app-token wird nicht aktualisiert, wenn gelöscht.

=== Spring boot interner server
mvn spring-boot:run "-Dwebpack.env=integration"

=== webpack dev server (Dynamische Seiten Update)
yarn start:integration




http://localhost:8888/ui/search
http://localhost:8888/esp-mykn-components/
