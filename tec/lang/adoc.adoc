...
* https://google.de[a external link]
* link:../lang/adoc.adoc[a internal checked link]
* p5470 ../lang/notExisting.adoc[a internal unchecked link]
* Wasser mit tiefer Zwei: H~2~O
...
[plantuml, message-sequence-diagram, png]
----
@startuml
actor User
participant Server
...
Server -> Server: Process Request (thinking)
... Processing ...
Server -> User: Send Response
deactivate Server

User -> Server: Short Request
Server -> User: Short Response
deactivate Server
@enduml
----
